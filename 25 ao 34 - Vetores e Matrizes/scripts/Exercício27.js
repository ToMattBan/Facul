...
function Media() { // Esta função faz o cálculo da média e mais outras coisas
	...
	if (preenchErradoo) {
		...
		if (mediaErrada) { } else {
			var mediaTurma = 0; // Criando varíaveis aqui para que não crie sem necessidade
			var acimaMedia = "";

			for (x = 1; x <= contaElementos; x++) {// Calculando a média
				mediaTurma += parseFloat(medias[x]);
				if (medias[x] >= 7) {
					acimaMedia += nomes[x] + "\n";
				}
			}
			mediaTurma = mediaTurma / contaElementos;



			// Saída
			document.getElementById("respostas").innerText = "A média da turma foi » " + mediaTurma + "\n\nOs alunos acima da média são »\n" + acimaMedia;
		}
	}
};